...
const VerificationApple = () => {
  ...
  const [confirmRegister, confirmRegisterStatus] = useConfirmAppleRegisterMutation();
  ...
  useEffect(() => {
    if (
      ...
      confirmRegisterStatus.isUninitialized
    ) {
      confirm();
    }
    if (checkTokenResult.error) {
      const { error } = checkTokenResult;
      message.open({
        tit: t('Error'),
        msg: (error as any)?.data?.message || t('Unknown_error'),
        rightBtn: t('Back_to_login'),
        rightBtnClick: onBackToLogin,
      });
    }
  }, [checkTokenResult, confirmRegisterStatus]);
...
export default VerificationApple;
